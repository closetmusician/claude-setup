<!-- ABOUTME: ceo-review/reference.md — Full procedural detail for the ceo-review skill. -->
<!-- ABOUTME: The gstack operational preamble, Voice spec, cognitive patterns, every Step 0 -->
<!-- ABOUTME: sub-step, all 11 review-section checklists, the outside-voice flow, required-output -->
<!-- ABOUTME: templates, review dashboard, and mode quick-reference. SKILL.md is the map; this is -->
<!-- ABOUTME: the terrain. Read this before running a real review. -->

# ceo-review — Full Procedure (reference)

This is the complete section-by-section procedure for the CEO/founder-mode plan review.
SKILL.md holds the purpose, boundaries, inputs, and the numbered overview; this file holds
the mechanics for every step. Run the Preamble first, then Step 0, then Sections 1-11, then
the outside voice and required outputs. Optional input: a design doc from a prior design/
planning session (if available) — see the Pre-Review System Audit "Design doc check".
...
if [ "${_TEL:-off}" != "off" ]; then
  echo '{"skill":"ceo-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
`~/.claude/skills/[skill-name]/SKILL.md` for reading skill files.
...
Key routing rules:
- Product ideas, "is this worth building", brainstorming → invoke prd-writer or eng-planning
...
If B: run `~/.claude/skills/bin/gstack-config set routing_declined true`
...
if [ "$_TEL" != "off" ]; then
  ...
  if [ -x ~/.claude/skills/bin/gstack-telemetry-log ]; then
...
|--------|---------|-----|------|--------|----------|
| CEO Review | \`/ceo-review\` | Scope & strategy | 0 | — | — |
...
```
If a design doc exists (from a prior design/planning session), read it. Use it as the source of truth for the problem statement, constraints, and chosen approach. If it has a `Supersedes:` field, note that this is a revised design.
...
from a prior CEO review session that paused so the user could produce a design doc. Use it
...
## Prior Design Doc Input (optional)

When the design doc check above prints "No design doc found," note the optional input
before proceeding — do not block on it.

Tell the user (one line, no AskUserQuestion needed): "No prior design doc found for this
branch. A design doc from a prior design/planning session — a structured problem statement,
premise challenge, and explored alternatives — would give this review sharper input, but
it's optional. Proceeding with standard review." Then proceed normally. Do not re-offer
later in the session.

If the user wants to produce one first, the closest existing skills are `eng-planning`
(feature design doc) or `prd-writer` (product framing) — they can run one of those and then
re-run this review; on return, re-run the design doc check and read whatever was produced.
The design doc is per-feature, not per-product — it captures the thinking behind this
specific change.

**Mid-session detection:** During Step 0A (Premise Challenge), if the user can't
articulate the problem, keeps changing the problem statement, answers with "I'm not
sure," or is clearly exploring rather than reviewing — surface it gently:

> "It sounds like you're still figuring out what to build — that's totally fine, but this
> review is sharpest once the problem is settled. If you want, pause here, capture the
> thinking in a design doc (e.g. via /eng-planning or /prd-writer), and re-run this review.
> Or we keep going as-is."

If they keep going, proceed normally — no guilt, no re-asking. Note current Step 0A
progress so you don't re-ask questions already answered.
...
if [ "$_CROSS_PROJ" = "true" ]; then
  ~/.claude/skills/bin/gstack-learnings-search --limit 10 --cross-project 2>/dev/null || true
else
  ~/.claude/skills/bin/gstack-learnings-search --limit 10 2>/dev/null || true
fi
```
...
Generated by /ceo-review on {date}
...
```bash
mkdir -p ~/.gstack/analytics
echo '{"skill":"ceo-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> ~/.gstack/analytics/spec-review.jsonl 2>/dev/null || true
```
...
```bash
~/.claude/skills/bin/gstack-review-log '{"skill":"ceo-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE","scope_proposed":N,"scope_accepted":N,"scope_deferred":N,"commit":"COMMIT"}'
```
...
- **Outside Voice (optional):** Independent plan review from a different AI model. Offered after all review sections complete in /ceo-review and /plan-eng-review. Falls back to Claude subagent if Codex is unavailable. Never gates shipping.
...
- **ceo-review**: \`status\`, \`unresolved\`, \`critical_gaps\`, \`mode\`, \`scope_proposed\`, \`scope_accepted\`, \`scope_deferred\`, \`commit\`
...
| Review | Trigger | Why | Runs | Status | Findings |
|--------|---------|-----|------|--------|----------|
| CEO Review | \`/ceo-review\` | Scope & strategy | {runs} | {status} | {findings} |
