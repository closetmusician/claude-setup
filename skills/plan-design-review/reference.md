<!--
ABOUTME: Full reference for the /plan-design-review skill — complete workflow detail + the
ABOUTME: shared gstack runtime preamble. SKILL.md is the compressed operational
ABOUTME: guide; this file is the load-on-demand source of truth for every phase,
ABOUTME: checklist, prompt, and rule. Preserved verbatim from the pre-compression
ABOUTME: SKILL.md (fable-skills packet 018). SKILL.md points here for all detail.
-->

# /plan-design-review — Full Reference

> SKILL.md holds the compressed workflow. This file holds the complete original:
> the gstack runtime **Preamble** (run at skill start, per SKILL.md) plus every
> workflow phase, checklist, and rule verbatim. Nothing here was dropped in compression.
...
~/.claude/skills/bin/gstack-timeline-log '{"skill":"plan-design-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
5. Tell the user: "Done. Each developer now runs: `cd ~/.claude && gstack setup --team`"
...
```bash
~/.claude/skills/bin/gstack-question-log '{"skill":"plan-design-review","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
**Log the result:**
```bash
~/.claude/skills/bin/gstack-review-log '{"skill":"design-outside-voices","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
~/.claude/skills/bin/gstack-review-log '{"skill":"plan-design-review","timestamp":"TIMESTAMP","status":"STATUS","initial_score":N,"overall_score":N,"unresolved":N,"decisions_made":N,"commit":"COMMIT"}'
```
...
```bash
~/.claude/skills/bin/gstack-learnings-log '{"skill":"plan-design-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
