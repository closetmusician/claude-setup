...
# 8. Gather design rationale signals (for Design Rationale section)
#    Collect anything that reveals WHY decisions were made, not just WHAT changed:
#    a. Full commit messages (not just --oneline): git log --format="%H%n%B" $BASE_BRANCH..$BRANCH
#       Look for: "considered X but chose Y because...", "tradeoff:", "alternative:", "why:"
#    b. PR description (if PR exists): gh pr view <number> --json body -q '.body'
#    c. Linked issues/specs: check commit messages for issue refs, then read the discussion
#    d. Code comments with rationale: grep -rn "TODO\|HACK\|NOTE\|TRADEOFF\|WORKAROUND\|chose.*because\|instead of" <changed-files>
#    e. Agent thinking traces: if commits reference Claude/Copilot/agent sessions, note that
#       reasoning was likely discarded — flag this gap explicitly in the briefing.
#    Purpose: reconstruct the decision log so reviewers validate documented reasoning
#    instead of reverse-engineering unstated assumptions (per Addy Osmani's agentic review model).

# 9. Compute LOC by semantic category (for Diff Composition table)
#    Using the --numstat output from step 4, bucket every file into one of:
#      a. New production code — non-test, non-doc, non-config, non-generated files with net additions
#      b. Deleted/replaced code — files with large deletions (net negative or refactor-replaced)
#      c. Tests — files matching test_*, *.test.*, *.spec.*, or in tests/ directories
#      d. Documentation — *.md files in docs/, design docs, READMEs
#      e. Data/config — JSON data files, YAML, .env, init files, lockfiles
#      f. Generated/skip — __pycache__, dist/, build artifacts, screenshots
#    For each category, sum additions + deletions and compute percentage of total diff.
#    For "deleted/replaced": when a file has >>deletions and <<additions (e.g., seed_skills.py
#    had +115/-2267), count the DELETIONS as "deleted code" and only the ADDITIONS as
#    "new production code". This prevents deleted legacy code from inflating the review surface.
```
...
### Phase 2.3: LOC Triage (Diff Composition)

Raw diff stats (e.g., "+5,216 / -2,321") are misleading for large PRs. A PR that deletes 2,000 lines of legacy code and adds 3,000 lines of tests looks massive but may contain only ~300 lines of new production code. This phase decomposes the diff into human-meaningful categories so reviewers know where to focus.

**Procedure:**

1. **Classify every file's additions and deletions into categories.** Use the file buckets from Phase 2 plus these additional split rules:
   - **Refactor splits:** When a single file has both large additions AND large deletions (e.g., a rewrite), count the deletions as "Deleted/replaced code" and only the net new additions as "New production code". The old code being removed is not reviewable — it's gone.
   - **Data files:** JSON, YAML, CSV, or similar structured data files that are not config (e.g., `default_experts.json`, seed data, fixture data) get their own "Data" category. These are spot-checked for structure, not line-by-line reviewed.

2. **Sum each category.** For each category, compute:
   - Total lines (additions + deletions touching files in that category)
   - Percentage of total diff
   - Reviewer action: **Review carefully**, **Skim/skip**, **Spot-check**, **Read for context**, **Skip**

3. **Write the headline callout.** If the ratio of new production code to total diff is <30%, write a prominent callout:
   > **This PR shows +X / -Y lines, but actual new production code is ~Z lines.**
   This prevents reviewer fatigue on PRs inflated by deletions, tests, or data.

4. **Write the compact chunk summary table.** For each review chunk (from Phase 2.5), add a row with: chunk name, key files, new LOC, estimated review time, and dependencies. Time estimates:
   - <100 lines of production code: ~2-3 min
   - 100-300 lines: ~5 min
   - 300-500 lines: ~10 min
   - 500+ lines: ~15-20 min
...
```markdown
## Summary
- [3-4 bullet points: what this PR does. Plain language, no file paths.]

**Jira:** [ticket link if found]
**Design doc:** [path if found]
**[N] files, +[X] / -[Y] lines | [N] tests pass**
...
[Only include the callout line if new production code is <50% of total diff. Otherwise omit it.]

### Diff Composition

| Category | Lines | % of diff | Action |
|---|---|---|---|
| New production code | ~NNN | NN% | **Review carefully** |
| Deleted/replaced code | ~NNN | NN% | Skim/skip |
| Tests (N new test files) | ~NNN | NN% | Spot-check |
| Documentation | ~NNN | NN% | Read for context |
| Data/config files | ~NNN | NN% | Skip |

[Omit any category with 0 lines. Merge categories with <2% into "Other".]

### [N] Review Chunks (~NN min total for production code)

| Chunk | Files | New Lines | Time | Dependencies |
|---|---|---|---|---|
| **A. [Name]** | `file1`, `file2` | NNN | ~N min | None (start here) |
| **B. [Name]** | `file3`, `file4` | NNN new, NNN deleted | ~N min | Depends on A |
| **C. [Name]** | `file5` | NNN | ~N min | None |

[This is the compact summary. Full chunk details with checklists appear in the Review Chunks section below.]

### Suggested Review Order

```
1. file1.py          — Leaf dependency. All validation lives here.
2. file2.json        — Data file validated by #1.
3. file3.py          — Core logic. Imports from #1.
```

[Numbered list, one line per file, with a short reason. Follows the import graph: leaves first, consumers last.]
...
[Capture the reasoning that would otherwise be lost — especially for agent-generated code where thinking traces are discarded after the diff is produced. Reviewers should validate documented reasoning, not reverse-engineer unstated assumptions.]

**Approach chosen:** [1-2 sentences: the high-level design approach this PR takes.]

**Key decisions:**
| Decision | Chosen | Alternatives considered | Why this won |
|----------|--------|------------------------|--------------|
| [e.g., "State management"] | [e.g., "Server-side session store"] | [e.g., "JWT tokens, client-side state"] | [e.g., "Need to revoke sessions instantly on permission change; JWTs can't be revoked without a blocklist that adds equivalent complexity"] |
| [Decision 2] | [Choice] | [Alternatives] | [Rationale] |

[Populate from: commit message bodies, PR description, linked issue discussions, code comments with rationale (TODO/NOTE/TRADEOFF), and design docs. If the PR was agent-generated and no rationale trail exists, state: "⚠️ No decision log found — this PR appears to be agent-generated with reasoning traces discarded. Reviewers must reconstruct intent from the code itself." This is the most expensive scenario for reviewers; flag it prominently.]

**Constraints driving the design:** [List any non-obvious constraints: backward compatibility, performance targets, API contracts, compliance requirements, dependency limitations. Omit if none found.]
...
- **Config source:** Deployment-specific defaults come from `.acme/catalog/app-workspace.yaml`. Env vars override. No hardcoded URLs or email addresses.
...
- **PR is dominated by deletions** (e.g., removing legacy code): In the Diff Composition table, show the deleted lines as their own category with "Skim/skip" action. The headline callout should emphasize how little new code there is. Example: "This PR shows +5,200 / -2,300 lines, but actual new production code is ~350 lines. The rest is deleted legacy dicts (2,270 lines) and tests (3,715 lines)."
- **PR includes large data files** (JSON fixtures, seed data, i18n): Classify data files as "Data" in Diff Composition, not "New production code". These are spot-checked for structure, not reviewed line-by-line. A 242-line JSON file of persona definitions is not 242 lines of reviewable code.
...
- Don't treat all additions as equal review surface. A 242-line JSON data file, a 652-line design doc, and a 700-line test file are NOT the same as 1,594 lines of production code. The Diff Composition table exists to prevent this mistake. If the raw diff says "+5,000" but only 350 lines are new production code, the briefing MUST surface that ratio prominently.
- Don't skip the Diff Composition table for small PRs. Even a 200-line PR benefits from knowing "120 lines are tests, 80 lines are implementation." The table is always mandatory.
- Don't leave the Design Rationale section empty or fill it with vague platitudes like "chose the simplest approach." If you can't find rationale signals (commit bodies, PR description, code comments, specs), say so explicitly — that missing-rationale warning IS the value. Reviewers need to know when they're flying blind.
- Don't fabricate rationale. If the commits are one-line messages and there's no design doc, don't invent "the author likely considered X." State what you found and what's missing. Fabricated rationale is worse than no rationale — it gives reviewers false confidence.
