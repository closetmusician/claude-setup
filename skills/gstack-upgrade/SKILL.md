...
[ -z "$_AUTO" ] && _AUTO=$(~/.claude/skills/bin/gstack-config get auto_upgrade 2>/dev/null || true)
...
```bash
~/.claude/skills/bin/gstack-config set auto_upgrade true
```
...
**If "Never ask again":**
```bash
~/.claude/skills/bin/gstack-config set update_check false
```
Tell user: "Update checks disabled. Run `~/.claude/skills/bin/gstack-config set update_check true` to re-enable."
...
_TEAM_MODE=$(~/.claude/skills/bin/gstack-config get team_mode 2>/dev/null || echo "false")
...
```bash
~/.claude/skills/bin/gstack-update-check --force 2>/dev/null || \
