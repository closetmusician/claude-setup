...
# Import the canonical field contract shared with session-journal.py.
# Replaces the ad-hoc inline parsing in parse_journal_entries() with the
# contract-aware parse_signal_from_block(). See scripts/lib/journal-schema.py.
_SCHEMA_PATH = Path(__file__).parent / "lib" / "journal-schema.py"
if _SCHEMA_PATH.exists():
    import importlib.util as _ilu
    _spec = _ilu.spec_from_file_location("journal_schema", _SCHEMA_PATH)
    _journal_schema = _ilu.module_from_spec(_spec)
    _spec.loader.exec_module(_journal_schema)
    _parse_signal_from_block = _journal_schema.parse_signal_from_block
else:
    # Fallback: treat every entry as having signal (safe degradation).
    def _parse_signal_from_block(block: str) -> dict:  # type: ignore[misc]
        return {
            "has_intent": True, "has_actions": True, "has_decisions": False,
            "has_errors": False, "has_struggles": False, "has_files": False,
            "has_signal": True, "is_new_format": True,
        }
...
def parse_journal_entries(content):
    ...
    Gotchas: Signal detection delegates to journal-schema.parse_signal_from_block() —
             the canonical contract shared with session-journal.py. Old-format
             (**Files:**, **Tools:**) dead-code branches have been removed; the bash
             auto-journal.sh writer is dead and no new entries use that format.
    ...
    for block in raw_blocks:
        ...
        # Delegate signal detection to the canonical schema module.
        # This replaces the ad-hoc inline field checks and removes the dead
        # old-format (**Files:**, **Tools:**) branches.
        signal_info = _parse_signal_from_block(block)
        ...
        entries.append(
            {
                ...
                "is_new_format": signal_info["is_new_format"],
            }
        )
    return entries
