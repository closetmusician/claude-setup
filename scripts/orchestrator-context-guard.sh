...
# ABOUTME: Sentinel-gated: inert unless $STATE/.orchestration-active or a $STATE/.gate-* sentinel exists.
...
# ABOUTME: All other files: hard deny at >500 lines. No warn-only — delegate or don't read.
...
# ─── Sentinel gate ───
# Guard is for orchestration sessions; it blocked 11+ user-requested reads in normal sessions (audit 2026-07-02).
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/governance/lib/project-root.sh"
GIT_ROOT="$(get_project_root)"
STATE="$(get_governance_state_dir)"
SENTINEL_PRESENT=0
if [[ -e "$STATE/.orchestration-active" || -e "$STATE/.active" ]]; then  # .active = sibling governance arm signal
  SENTINEL_PRESENT=1
else
  # No nullglob dependency: unmatched glob stays literal and fails the -e test (bash 3.2 / BSD safe).
  for f in "$STATE"/.gate-*; do
    if [[ -e "$f" ]]; then
      SENTINEL_PRESENT=1
      break
    fi
...
# Normalize: expand leading ~ and strip trailing slashes for consistent matching.
case "$FILE_PATH" in
  "~") FILE_PATH="$HOME" ;;
...
Task(description='Extract from ${FILE_PATH##*/}', subagent_type='Explore', prompt='Read ${FILE_PATH} in full, then answer: [YOUR SPECIFIC QUESTION].
...
# Everything else: hard deny at >500 lines. No warn-only — delegate or don't read.
if [[ "$LINE_COUNT" -gt 500 ]]; then
  deny_and_exit "ORCHESTRATOR CONTEXT GUARD: ${FILE_PATH} is ${LINE_COUNT} lines. ${DENY_MSG_SUFFIX}"
fi

# ≤500 lines, not code, not spec: allow.
exit 0
