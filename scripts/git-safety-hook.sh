...
_GIT_EMIT_SH="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/lib/emit-event.sh"
# shellcheck disable=SC1090
[[ -f "$_GIT_EMIT_SH" ]] && source "$_GIT_EMIT_SH" 2>/dev/null || true
unset _GIT_EMIT_SH
...
deny_and_exit() {
  ...
  emit_event "incident" '{"class":"git_safety_deny","guard":"git-safety-hook"}' \
    outcome="denied" source="git-safety-hook.sh" || true
  jq -n \
    --arg reason "$reason" \
    '{
      "hookSpecificOutput": {
