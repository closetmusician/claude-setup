...
# Import the canonical field contract. Both writer and reader use this module
# so the schema stays in lock-step. See scripts/lib/journal-schema.py.
...
if _SCHEMA_PATH.exists():
    ...
    _is_noise_entry = _journal_schema.is_noise_entry
else:
    # Fallback if schema module is missing — never skip entries (safe degradation).
    def _is_noise_entry(outcome: str, block: str) -> bool:  # type: ignore[misc]
        return False
...
def main() -> None:
    ...
    # De-noise at source: skip pure 'session ended' entries with no signal.
    # Kills the ~46% noise ratio without losing any learnable content.
    # Signal definition lives in scripts/lib/journal-schema.py::is_noise_entry().
    if _is_noise_entry(outcome, entry):
        return
