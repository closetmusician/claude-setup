...
# ─── Resolve script dir early (needed for lib sourcing) ───
...
# ─── State directory ───
STATE_DIR="$(get_governance_state_dir)"
mkdir -p "$STATE_DIR"
...
if [[ "$RED_COUNT" -eq 0 ]]; then
  ...
  # Emit agent_result on PASS — purely additive.
  emit_event "agent_result" "{\"task_id\":\"${TASK_ID}\",\"result\":\"PASS\",\"green\":${GREEN_COUNT},\"red\":0}" \
    outcome="ok" source="post-agent-audit.sh" || true
  exit 0
fi
...
# Emit agent_result on FAIL — purely additive, before the block output.
emit_event "agent_result" "{\"task_id\":\"${TASK_ID}\",\"result\":\"FAIL\",\"green\":${GREEN_COUNT},\"red\":${RED_COUNT}}" \
  outcome="denied" source="post-agent-audit.sh" || true
