...
# Emit agent_spawn on allow — purely additive, after all state is persisted.
TASK_ID_EMIT=$(echo "$REQ_MAP_JSON" | jq -r '.task_id // "unknown"' 2>/dev/null || echo "unknown")
emit_event "agent_spawn" "{\"task_id\":\"${TASK_ID_EMIT}\",\"gate\":\"pre-agent-gate\"}" \
  outcome="allowed" source="pre-agent-gate.sh" || true
