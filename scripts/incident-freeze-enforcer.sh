...
_ENFORCE_EMIT_SH="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/lib/emit-event.sh"
# shellcheck disable=SC1090
[[ -f "$_ENFORCE_EMIT_SH" ]] && source "$_ENFORCE_EMIT_SH" 2>/dev/null || true
unset _ENFORCE_EMIT_SH
...
deny_and_exit() {
  local reason="$1"
  # Emit incident on deny — purely additive; before the deny JSON output.
  emit_event "incident" '{"class":"freeze_enforced","cause":"external_mutation_blocked"}' \
    outcome="denied" source="incident-freeze-enforcer.sh" || true
  jq -n \
    --arg reason "$reason" \
    '{
      "hookSpecificOutput": {
